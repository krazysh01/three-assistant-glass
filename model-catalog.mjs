// Model and voice discovery for OpenAI-compatible servers.
//
...
//
// So everything here is best-effort and degrades: use the richer fields when the
// server provides them, fall back to a documented list for known OpenAI models,
// and otherwise return nothing so the field stays free text. Suggestions are
// never a constraint - a valid model or voice that isn't listed must still be
// typeable, since any bundled list goes stale the moment a provider adds one.
...
// Fetch the model list from an OpenAI-compatible server. Returns [] on any
// failure - server down, no CORS, endpoint absent - so callers just get no
...
export async function fetchModels(baseUrl, apiKey) {
  ...
  try {
    ...
    console.warn(`[models] no suggestions from ${url}:`, e.message);
    return [];
  }
}

// Models for one task. A server that reports `task` is filtered properly; one
// that doesn't (OpenAI, and anything minimal) returns everything, since a full
// list is more useful than a wrong guess about which models transcribe.
