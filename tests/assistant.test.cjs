...
async function speakerHarness() {
  ...
  const module = await load('assistant/tts.js', { AudioContext }, {
    'speech.mjs': {
      synthesize: (text, _settings, { signal } = {}) => {
  ...
  const ready = text => requests.get(text).resolve(text); // decodeAudioData is identity here
...
async function pipelineHarness(settings = {}, startup = Promise.resolve()) {
  ...
  const module = await load('assistant/pipeline.js', {}, {
    ...
    'assistant/llm.js': { streamChat: (messages, _settings, options) => {
  ...
  const assistant = module.createAssistant(settings, {
    onText() {}, onSpeaker() {}, onStatus: text => statuses.push(text),
    onError: err => errors.push(err), onEnd: err => ends.push(err),
  });
...
test('stopping during assistant startup suppresses the greeting and listening status', async () => {
  ...
  const h = await pipelineHarness({ customFirstMessage: 'Hello!' }, startup.promise);
...
test('a denied microphone is fatal, not a routine recognition error', async () => {
  ...
  }, { 'speech.mjs': { floatToPcm16: s => s, transcribe: async () => '' } });
...
test('transcription completing after stop cannot produce another utterance', async () => {
  ...
  const module = await load('assistant/stt.js', {}, {
    ...
    'speech.mjs': {
      floatToPcm16: samples => samples,
      transcribe: (_pcm, _settings, options) => { signal = options.signal; return pending.promise; },
    },
  });
  ...
  pending.resolve('late transcript'); await transcribing;
  assert.equal(utterances, 0);
});
...
function mainHarness(createAssistant) {
  ...
  const context = vm.createContext({
    console, createAssistant, currentVrm: null, characterExpressions: { supported: () => [], apply() {}, reset() {} },
    window: { addEventListener() {} },
    document: { getElementById: id => id === 'toggleVapi' ? button : statuses },
    updateTextMesh() {}, updateVrmNameDisplay() {},
    loadSettings: () => { const request = deferred(); requests.push(request); return request.promise; },
  });
...
test('Stop during settings fetch prevents a late session from starting', async () => {
  ...
  h.requests[0].resolve({}); await starting;
...
test('failure from a stopped startup cannot stop a newer session', async () => {
  ...
  const starting = h.toggle(); h.requests[0].resolve({}); await tick();
  await h.toggle();
  const restarting = h.toggle(); h.requests[1].resolve({}); await restarting;
