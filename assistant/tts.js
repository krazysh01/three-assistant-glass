...
//
//   openai  → the configured /audio/speech endpoint, called from the browser
...
function createEndpointSynth(settings) {
  return async (text, audioCtx, signal) => {
    const audio = await synthesize(text, settings, { signal });
    signal?.throwIfAborted();
    return audioCtx.decodeAudioData(audio);
  };
}
